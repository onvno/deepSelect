...
function deepSelect (options) {
	...
	var selOri = selWrap.getElementsByTagName('select')[0];
	// 此处正则必须有大写，对付奇葩ie8
	...
	var selWrapOri = selWrap.innerHTML;
	selWrap.innerHTML = selWrap.innerHTML + addtCont;
	// 修正selOri因selWrap内容调整后无法取值的问题
	selOri = selWrap.getElementsByTagName('select')[0];
	 ...
	 *	new select init
	**/
	var selNewWrap = selWrap.querySelectorAll('.de_select')[0];
	// origin select hide
